...
class CleanRunner
  ...
  def self.delete
    ...
    # Check, golden_eagle path
    gold_exist = "golden_eagle"
    encoding_style
    ...
    while (line = Readline.readline(""))
      ...
      if line.match?(sel[0])
        ...
        puts ''
        puts 'Deleted, the existing golden_eagle log folder.'
        puts ''
        break
  ...
  def self.run
    ...
    if Dir.exist?(File.expand_path("~/#{gold_exist}"))
      puts ''
      puts 'Already have a golden_eagle log folder.'
      delete
    else
      FileUtils.mkdir('golden_eagle_log')
      FileUtils.mv("#{File.dirname(__FILE__)}/golden_eagle", File.expand_path('~/'))
      puts ''
      puts 'Created, golden_eagle log folder.'
      puts ''
    end
  end
end
